...
def left_join_df(df_table_1:pd.DataFrame,df_table_2:pd.DataFrame,foreign_key_1:str,foreign_key_2='')->pd.DataFrame:
    ...
    try:
        ...
        merged_df=pd.merge(df_table_1,df_table_2,left_on='table1_'+foreign_key_1,right_on='table2_'+foreign_key_2)
        return merged_df
    except Exception as e:
        print(e)
        return e

def column_filter(df_table:pd.DataFrame,columns_filter=[],columns_renamed=[])->pd.DataFrame:
    """
    ...
    try:
        ...
        if(len(columns_renamed)!=len(columns_filter)):
            raise ValueError('The columns to be filtered have a different length than the new names you want to assign to them with columns_renamed list')
        if False in [bool(field in df_table.columns.to_list()) for field in columns_filter]:
            raise Exception('The column name in columns_filter list is not found inside the data frame')
