...
def lambda_handler2(event, context):
    try:
        ...
        dataframes, counters_dates = tables_reader_from_s3(
            all_files_from_ingestion, INGESTION_BUCKET
        )
        ...
        upload_processed_df(
            processed_dataframes, counters_dates, s3, logger, PROCESSED_BUCKET
        )
        write_state_file("/tmp/state_file.json", state_file_dict)
        s3.upload_file(
            Filename=f"/tmp/state_file.json",
...
def process_dataframes(dataframes: dict[pd.DataFrame]) -> dict[pd.DataFrame]:
    ...
    state_file_dict = {
        ...
        "fact_payment": False,
        "dim_payment_type": old_statefile["payment_type"],
        "dim_currency": old_statefile["currency"],
        "dim_counterparty": old_statefile["counterparty"],
        "dim_location": old_statefile["address"],
    }
    try:
        if "sales_order" in dataframes:
            processed_df_dict["fact_sales_order"] = fact_sales_transformer(
                dataframes["sales_order"]
            )
            processed_df_dict["dim_date"] = transform_date_table(
                dataframes["sales_order"]
            )
            state_file_dict["fact_sales_order"] = True
            state_file_dict["dim_date"] = True
        if "staff" in dataframes:
            processed_df_dict["dim_staff"] = transform_staff_table(
                dataframes["staff"], dataframes["department"]
            )
            state_file_dict["dim_staff"] = True
        if "design" in dataframes:
            processed_df_dict["dim_design"] = transform_design_table(
                dataframes["design"]
            )
            state_file_dict["dim_design"] = True
        if "transaction" in dataframes:
            processed_df_dict["dim_transaction"] = dim_transaction(
                dataframes["transaction"]
            )
            state_file_dict["dim_transaction"] = True
        if "purchase_order" in dataframes:
            processed_df_dict["fact_purchase_order"] = transform_fact_purchase_order(
                dataframes["purchase_order"]
            )
        ...
        processed_df_dict["dim_payment_type"] = dim_payment_type(
            dataframes["payment_type"]
        )
        processed_df_dict["dim_currency"] = transform_currency_table(
            dataframes["currency"]
        )
        processed_df_dict["dim_counterparty"] = dim_counterparty(
            dataframes["counterparty"], dataframes["address"]
        )
        processed_df_dict["dim_location"] = transform_location_table(
            dataframes["address"]
        )
